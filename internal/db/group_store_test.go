...
func TestRoutingGroupsRepositoryRoundTripsDefinitionAndRevision(t *testing.T) {
	...
	loaded, err := store.Groups.Get(context.Background(), created.ID)
	if err != nil {
		t.Fatal(err)
	}
	...
	updated, err := store.Groups.Save(context.Background(), groups.Definition{ID: created.ID, Name: "Coding v2", Slug: "coding-v2", Enabled: true, Stages: loaded.Stages}, &created.Revision)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := store.Groups.Save(context.Background(), groups.Definition{ID: created.ID, Name: "stale", Slug: "stale", Enabled: true, Stages: loaded.Stages}, &created.Revision); err == nil {
...
func TestDeleteReferencedGroupIsProtectedByForeignKey(t *testing.T) {
	...
	defer store.Close()
	child, err := store.Groups.Save(context.Background(), groups.Definition{Name: "Child", Slug: "child", Enabled: true, Stages: []groups.Stage{{Position: 0, Sources: []groups.Source{{Kind: groups.SourceModel, ModelID: "model-a"}}, BillingClasses: []groups.BillingClass{groups.BillingMetered}}}}, nil)
	if err != nil {
		t.Fatal(err)
	}
	parent, err := store.Groups.Save(context.Background(), groups.Definition{Name: "Parent", Slug: "parent", Enabled: true, Stages: []groups.Stage{{Position: 0, Sources: []groups.Source{{Kind: groups.SourceGroup, GroupID: child.ID}}, BillingClasses: []groups.BillingClass{groups.BillingMetered}}}}, nil)
	if err != nil {
		t.Fatal(err)
	}
	if err := store.Groups.Delete(context.Background(), child.ID, child.Revision); err == nil {
		t.Fatal("expected foreign-key delete failure")
	}
	if _, err := store.Groups.Get(context.Background(), parent.ID); err != nil {
		t.Fatal(err)
	}
}
