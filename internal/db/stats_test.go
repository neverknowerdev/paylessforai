...
func TestAttemptStatsIncludeDurationForFailedAttempt(t *testing.T) {
	...
	defer s.Close()
	if err := s.CreateProxyRequest(context.Background(), "failed-request", "", "chat_completions", "model-a"); err != nil {
		t.Fatal(err)
	}
	if err := s.RecordProxyAttempt(context.Background(), "failed-request", 1, "openrouter", "model-a", "started", "", ""); err != nil {
		t.Fatal(err)
	}
	time.Sleep(2 * time.Millisecond)
	if err := s.RecordProxyAttempt(context.Background(), "failed-request", 1, "openrouter", "model-a", "failed", "provider_error", "Provider failed"); err != nil {
		t.Fatal(err)
	}
	items, err := s.ListRequestStats(context.Background(), 10)
	if err != nil {
