...
func (r *CatalogRepository) ListPricing(ctx context.Context, query string, limit, offset int) ([]models.PricingRow, int, error) {
	...
	if query != "" {
		...
		where = "WHERE m.normalized_name ILIKE $1 OR m.display_name ILIKE $1 OR m.canonical_slug ILIKE $1"
	}
	countQuery := `SELECT count(DISTINCT m.id) FROM models m LEFT JOIN provider_offerings o ON o.model_id=m.id ` + where
	var total int
	...
	rows, err := r.db.QueryContext(ctx, `SELECT p.model_id,p.offering_id,p.canonical_slug,p.display_name,p.provider,p.provider_model_id,p.input_usd_per_million,p.output_usd_per_million,p.cache_read_usd_per_million,p.cache_write_usd_per_million,p.official_input_usd_per_million,p.official_output_usd_per_million,p.official_cache_read_usd_per_million,p.official_cache_write_usd_per_million,p.official_price_source,p.official_price_source_url,p.official_price_observed_at,p.override_input_usd_per_million,p.override_output_usd_per_million,p.override_cache_read_usd_per_million,p.override_cache_write_usd_per_million,p.override_updated_at FROM (SELECT DISTINCT ON (m.id) m.id AS model_id,COALESCE(o.id,0) AS offering_id,m.canonical_slug,m.display_name,COALESCE(o.provider,'') AS provider,COALESCE(o.provider_model_id,'') AS provider_model_id,COALESCE(o.override_input_usd_per_million,o.official_input_usd_per_million,o.input_usd_per_million) AS input_usd_per_million,COALESCE(o.override_output_usd_per_million,o.official_output_usd_per_million,o.output_usd_per_million) AS output_usd_per_million,COALESCE(o.override_cache_read_usd_per_million,o.official_cache_read_usd_per_million,o.cache_read_usd_per_million) AS cache_read_usd_per_million,COALESCE(o.override_cache_write_usd_per_million,o.official_cache_write_usd_per_million,o.cache_write_usd_per_million) AS cache_write_usd_per_million,o.official_input_usd_per_million,o.official_output_usd_per_million,o.official_cache_read_usd_per_million,o.official_cache_write_usd_per_million,COALESCE(o.official_price_source,'') AS official_price_source,COALESCE(o.official_price_source_url,'') AS official_price_source_url,o.official_price_observed_at,o.override_input_usd_per_million,o.override_output_usd_per_million,o.override_cache_read_usd_per_million,o.override_cache_write_usd_per_million,o.override_updated_at FROM models m LEFT JOIN provider_offerings o ON o.model_id=m.id `+where+` ORDER BY m.id,CASE o.official_price_source WHEN 'Artificial Analysis' THEN 0 WHEN 'OpenRouter' THEN 1 WHEN 'Surplus Intelligence' THEN 2 ELSE 3 END,o.observed_at DESC NULLS LAST,o.id DESC) p ORDER BY p.display_name LIMIT $`+strconv.Itoa(limitPlaceholder)+` OFFSET $`+strconv.Itoa(offsetPlaceholder), args...)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
