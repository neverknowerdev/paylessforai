(() => {
  const state = { requests: [], models: [], modelStats: [], providerStats: [], summary: {}, view: 'overview', detailDrawer: null, modelSort: { key: null, direction: null }, modelFilters: { model: [], provider: [], tags: [], input: [], output: [] }, filterPopover: null };
  ...
  function updateProviderFormMode() { const custom = $('#provider-type')?.value === 'custom'; const fields = $('#custom-provider-fields'); const name = $('#provider-name'); const baseURL = $('#provider-base-url'); if (fields) fields.hidden = !custom; if (name) { name.required = custom; if (!custom) name.value = ''; } if (baseURL) { baseURL.required = custom; if (!custom) baseURL.value = ''; } }
  ...
  function setProviderSaving(saving) { const form = $('#provider-form'); const progress = $('#provider-progress'); if (!form) return; form.setAttribute('aria-busy', saving ? 'true' : 'false'); form.querySelectorAll('input, select, button[type="submit"]').forEach((control) => { control.disabled = saving; }); if (progress) progress.hidden = !saving; }
  $('#provider-form').addEventListener('submit', async (event) => { event.preventDefault(); const custom = $('#provider-type').value === 'custom'; setProviderSaving(true); try { await fetchJSON('/api/providers/credentials', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ provider: custom ? $('#provider-name').value : $('#provider-type').value, base_url: custom ? $('#provider-base-url').value : '', label: $('#provider-label').value, api_key: $('#provider-key').value }) }); $('#provider-form').reset(); updateProviderFormMode(); closeModal('provider-modal'); await Promise.all([loadProviders(), loadModels(), loadStatus()]); } catch (error) { window.alert(error.message); } finally { setProviderSaving(false); } });
  $('#open-key-modal').addEventListener('click', () => openModal('key-modal')); $('#open-provider-modal').addEventListener('click', () => { $('#provider-form').reset(); updateProviderFormMode(); openModal('provider-modal'); }); $('#provider-type').addEventListener('change', updateProviderFormMode); $('#menu-toggle').addEventListener('click', () => $('#sidebar').classList.toggle('open')); $('#refresh-button').addEventListener('click', () => Promise.all([loadStatus(), loadSummary(), loadRequests(), loadModelStats(), loadProviderStats(), loadModels(), loadKeys(), loadProviders()])); $('#models-refresh').addEventListener('click', loadModels);
  $('#models-search').addEventListener('input', renderModels); $('#models-clear-filters').addEventListener('click', clearModelFilters); $$('.table-sort').forEach((button) => button.addEventListener('click', () => cycleModelSort(button.dataset.sortKey))); $$('.table-filter').forEach((button) => button.addEventListener('click', (event) => { event.stopPropagation(); openModelFilter(button.dataset.filterKey, button); })); $('#requests-search').addEventListener('input', renderRequestTable); $('#requests-state').addEventListener('change', renderRequestTable);
  ...
  updateProviderFormMode(); setText('#base-url', `${window.location.origin}/v1`); Promise.all([loadStatus(), loadSummary(), loadRequests(), loadModelStats(), loadProviderStats(), loadModels(), loadKeys(), loadProviders()]);
})();
