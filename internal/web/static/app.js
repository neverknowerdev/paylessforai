(() => {
  ...
  function clearNewKey() { const secret = $('#new-key'); if (!secret) return; secret.hidden = true; const value = $('#new-key-value'); if (value) value.textContent = ''; }
  ...
  $('#key-form').addEventListener('submit', async (event) => { event.preventDefault(); const harness = $('#key-harness').value === 'Other' ? $('#key-harness-other').value.trim() : $('#key-harness').value; if (!harness) { $('#key-harness-other').focus(); return; } try { const payload = await fetchJSON('/api/client-keys', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ label: $('#key-label').value, harness }) }); const secret = $('#new-key'); $('#new-key-value').textContent = payload.secret; secret.hidden = false; $('#key-form').reset(); updateHarnessFormMode(); await loadKeys(); } catch (error) { const secret = $('#new-key'); secret.hidden = false; secret.textContent = error.message; } });
