(() => {
  ...
  const providerName = (value) => value === 'surplus' ? 'Surplus Intelligence' : 'OpenRouter';
  ...
  function renderRecentRequests() {
    ...
    state.requests.slice(0, 5).forEach((request) => {
      ...
      const main = document.createElement('span'); main.className = 'request-main'; const title = document.createElement('strong'); title.textContent = request.model; const detail = document.createElement('small'); detail.textContent = `${protocolName(request.protocol)} · ${dateValue(request.received_at)}`; main.append(title, detail); row.append(main);
  ...
  function filteredRequests() {
    ...
    return state.requests.filter((request) => (filter === 'all' || request.state === filter) && (!search || `${request.id} ${request.model} ${request.protocol}`.toLowerCase().includes(search)));
  }
  ...
  function showRequestDetail(id) {
    ...
    const grid = document.createElement('div'); grid.className = 'detail-grid'; const values = [['Model', request.model], ['Protocol', protocolName(request.protocol)], ['State', request.state], ['Input tokens', formatNumber(request.input_tokens)], ['Output tokens', formatNumber(request.output_tokens)], ['Cached read', formatNumber(request.cached_read_tokens)], ['Cache write', formatNumber(request.cache_write_tokens)], ['Reasoning', formatNumber(request.reasoning_tokens)], ['Estimated cost', formatUSD(request.estimated_cost_pico_usd)], ['Actual cost', request.actual_cost_pico_usd != null ? formatUSD(request.actual_cost_pico_usd) : 'Not reported'], ['Received', dateValue(request.received_at)], ['Completed', dateValue(request.completed_at)]];
  ...
  function filteredModels() { const search = ($('#models-search')?.value || '').trim().toLowerCase(); const provider = $('#models-provider-filter')?.value || 'all'; return state.models.filter((model) => (provider === 'all' || model.provider === provider) && (!search || `${model.model} ${model.upstream_model} ${model.provider}`.toLowerCase().includes(search))); }
