(() => {
  const state = { requests: [], models: [], modelStats: [], providerStats: [], groupStats: [], groups: [], providers: [], summary: {}, network: {}, view: 'overview', detailDrawer: null, modelSort: { key: null, direction: null }, modelFilters: { model: [], provider: [], tags: [], input: [], output: [] }, filterPopover: null };
  ...
  function isNewModel(model) { return model.routes.some((route) => route.is_new); }
  function renderModels() {
    const body = $('#models-table-body'); const empty = $('#models-empty'); if (!body) return; const models = filteredModels(); models.sort((a, b) => Number(isNewModel(b)) - Number(isNewModel(a))); body.replaceChildren(); const allModels = groupedModels(); const newCount = allModels.filter(isNewModel).length; setText('#topbar-model-count', `${formatNumber(allModels.length)} Models`); const newCountBadge = $('#topbar-new-model-count'); newCountBadge.textContent = `${newCount} new`; newCountBadge.hidden = newCount === 0; setText('#catalog-count', `${formatNumber(models.length)} model${models.length === 1 ? '' : 's'}`); setText('#catalog-free-count', `${formatNumber(state.models.length)} routes`); setText('#catalog-free-note', `${formatNumber(state.models.filter((model) => model.free).length)} free routes`); setText('#models-saved', formatUSD(state.summary?.saved_cost_pico_usd)); setText('#models-saved-percent', state.summary?.saved_percent_bps == null ? 'No baseline' : `${discountPercent(state.summary.saved_percent_bps)} saved`);
    models.forEach((model) => { const row = document.createElement('tr'); const route = appendTextCell(row, ''); const strong = document.createElement('strong'); strong.textContent = model.model; const small = document.createElement('small'); small.textContent = `${formatNumber(model.routes.length)} provider route${model.routes.length === 1 ? '' : 's'}`; route.append(strong, small); if (isNewModel(model)) { const badge = document.createElement('span'); badge.className = 'badge new-model'; badge.textContent = 'New'; badge.title = 'Added to this catalog in the last 7 days'; route.append(document.createTextNode(' '), badge); row.classList.add('new-model-row'); } if (model.free) { const badge = document.createElement('span'); badge.className = 'badge free'; badge.textContent = 'FREE'; route.append(document.createTextNode(' '), badge); }
