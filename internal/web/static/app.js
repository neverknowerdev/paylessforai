(() => {
  ...
  $('#provider-form').addEventListener('submit', async (event) => { event.preventDefault(); const custom = $('#provider-type').value === 'custom'; let manualModels = []; try { manualModels = parseManualModels(); } catch (error) { setProviderFeedback(error.message, 'error'); return; } setProviderFeedback(''); setProviderSaving(true); try { await fetchJSON('/api/providers/credentials', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ provider: custom ? $('#provider-name').value : $('#provider-type').value, base_url: custom ? $('#provider-base-url').value : '', label: $('#provider-label').value, api_key: $('#provider-key').value, manual_models: manualModels }) }); $('#provider-form').reset(); updateProviderFormMode(); $('#manual-model-fields').hidden = true; setProviderFeedback(''); closeModal('provider-modal'); await Promise.all([loadProviders(), loadModels(), loadStatus()]); } catch (error) { const details = error.payload?.error; if (details?.can_enter_models) { $('#manual-model-fields').hidden = false; setProviderFeedback(`${details.message} Enter model IDs and prices below; each will be verified before the credential is saved.`); } else { setProviderFeedback(error.message, 'error'); } } finally { setProviderSaving(false); } });
