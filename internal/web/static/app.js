(() => {
  const state = { requests: [], models: [], modelStats: [], providerStats: [], summary: {}, view: 'overview', detailDrawer: null, modelSort: { key: null, direction: null } };
  ...
  function renderProviderStats() {
    ...
    items.forEach((item) => {
      ...
      const tokens = appendTextCell(row, ''); const tokensStrong = document.createElement('strong'); tokensStrong.textContent = formatCompact(item.total_tokens); const tokensSmall = document.createElement('small'); tokensSmall.textContent = `${formatCompact(item.input_tokens)} in · ${formatCompact(item.output_tokens)} out · ${formatCompact(item.cached_read_tokens)} cached`; tokens.append(tokensStrong, tokensSmall);
