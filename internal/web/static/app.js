(() => {
  ...
  async function loadProviderOptions() { try { const payload = await fetchJSON('/api/providers'); const options = $('#provider-options'); if (!options) return; options.replaceChildren(); (payload.data || []).forEach((provider) => { const option = document.createElement('option'); option.value = provider.name; option.label = provider.display_name || provider.name; option.dataset.baseUrl = provider.default_base_url || ''; options.append(option); }); } catch (_) {} }
  ...
  $('#provider-form').addEventListener('submit', async (event) => { event.preventDefault(); try { await fetchJSON('/api/providers/credentials', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ provider: $('#provider-name').value, base_url: $('#provider-base-url').value, label: $('#provider-label').value, api_key: $('#provider-key').value }) }); $('#provider-form').reset(); closeModal('provider-modal'); await Promise.all([loadProviders(), loadModels(), loadStatus()]); } catch (error) { window.alert(error.message); } });
  $('#open-key-modal').addEventListener('click', () => openModal('key-modal')); $('#open-provider-modal').addEventListener('click', () => openModal('provider-modal')); $('#menu-toggle').addEventListener('click', () => $('#sidebar').classList.toggle('open')); $('#refresh-button').addEventListener('click', () => Promise.all([loadStatus(), loadSummary(), loadRequests(), loadModelStats(), loadProviderStats(), loadModels(), loadKeys(), loadProviders()])); $('#models-refresh').addEventListener('click', loadModels);
  ...
  setText('#base-url', `${window.location.origin}/v1`); Promise.all([loadStatus(), loadSummary(), loadRequests(), loadModelStats(), loadProviderStats(), loadModels(), loadKeys(), loadProviders(), loadProviderOptions()]);
})();
