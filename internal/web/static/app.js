(() => {
  const state = { requests: [], models: [], modelStats: [], summary: {}, view: 'overview', detailDrawer: null };
  ...
  const modelDiscount = (bps) => bps == null ? '—' : `${discountPercent(bps)} ${Number(bps) >= 0 ? 'saved' : 'overage'}`;
  ...
  async function loadSummary() {
    try {
      ...
      setText('#metric-saved-note', `Saved ${formatUSD(s.saved_cost_pico_usd)} vs. official price`);
  ...
  function renderStatsOverview() {
    ...
    const items = [['REQUESTS · RELIABILITY', s.total_requests ? `${formatNumber(s.total_requests)} · ${success}%` : '—', `${formatNumber(s.total_attempts || 0)} attempts · ${formatNumber(retried)} retried`, 'accent-peach'], ['RESPONSE TIME', formatDuration(s.average_response_ms), `${formatDuration(s.fastest_response_ms)} fastest · ${formatDuration(s.slowest_response_ms)} slowest`, 'accent-blue'], ['TOKENS', formatNumber(s.total_tokens), `${formatNumber(s.input_tokens || 0)} in · ${formatNumber(s.output_tokens || 0)} out · ${cacheHitPercent(s)} cache hit`, 'accent-lime'], ['SPEND · SAVED', formatUSD(s.actual_cost_pico_usd || s.estimated_cost_pico_usd), `Estimate ${formatUSD(s.estimated_cost_pico_usd)} · saved ${formatUSD(s.saved_cost_pico_usd)}`, 'accent-green']];
    items.forEach(([label, value, note, accent]) => { const card = document.createElement('article'); card.className = `metric-card ${accent}`.trim(); const title = document.createElement('span'); title.className = 'metric-label'; title.textContent = label; const number = document.createElement('strong'); number.textContent = value; const small = document.createElement('small'); small.textContent = note; card.append(title, number, small); container.append(card); });
  }
  ...
  function filteredModels() { const search = ($('#models-search')?.value || '').trim().toLowerCase(); const provider = $('#models-provider-filter')?.value || 'all'; const tag = $('#models-tag-filter')?.value || 'all'; const sort = $('#models-sort')?.value || 'model-asc'; const items = state.models.filter((model) => (provider === 'all' || model.provider === provider) && (tag === 'all' || (model.tags || []).includes(tag)) && (!search || `${model.model} ${model.upstream_model} ${model.provider} ${(model.tags || []).join(' ')} ${(model.input_modalities || []).join(' ')} ${(model.output_modalities || []).join(' ')}`.toLowerCase().includes(search))); const price = (model, key) => { const value = Number(model.pricing?.[key]); return model.price_available && Number.isFinite(value) ? value : null; }; const valueForSort = (model) => { if (sort.startsWith('discount')) return model.discount_percent_bps == null ? null : Number(model.discount_percent_bps); if (sort.startsWith('input')) return price(model, 'input'); if (sort.startsWith('output')) return price(model, 'output'); return String(model.model || '').toLowerCase(); }; const direction = sort.endsWith('desc') ? -1 : 1; return items.sort((a, b) => { const av = valueForSort(a); const bv = valueForSort(b); if (av == null && bv == null) return String(a.model).localeCompare(String(b.model)); if (av == null) return 1; if (bv == null) return -1; if (typeof av === 'string') return direction * av.localeCompare(bv); return direction * (av - bv); }); }
  ...
  $('#models-search').addEventListener('input', renderModels); $('#models-provider-filter').addEventListener('change', renderModels); $('#models-tag-filter').addEventListener('change', renderModels); $('#models-sort').addEventListener('change', renderModels); $('#requests-search').addEventListener('input', renderRequestTable); $('#requests-state').addEventListener('change', renderRequestTable);
