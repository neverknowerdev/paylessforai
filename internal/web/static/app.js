(() => {
  ...
  const formatUsage = (value) => `${formatCompact(value).toLowerCase()} reqs`;
  ...
  const updateActivePhases = new Set(['downloading', 'verified', 'staged', 'draining', 'snapshotting', 'preflighting', 'migrating', 'starting', 'stabilizing', 'rolling_back']);
  const updateTerminalPhases = new Set(['promoted', 'failed', 'rolled_back', 'needs_manual_recovery']);
  ...
  const updateActive = (phase) => updateActivePhases.has(String(phase));
  ...
  const updateTerminal = (phase) => updateTerminalPhases.has(String(phase));
  ...
  function ensureUpdatePolling(payload, modalIntent = false) {
    const stateUpdate = payload?.state || {};
    if (!updateActive(stateUpdate.phase)) return;
    if (!state.updateOperation?.active) {
      state.updateOperation = { active: true, polling: false, observedStart: true, modalIntent, initialOperationID: String(stateUpdate.operation_id || '') };
    } else if (modalIntent) {
      state.updateOperation.modalIntent = true;
    }
    if (!state.updateOperation.polling) pollUpdateProgress();
  }
  function showUpdateProgressModal(payload, remember = true) {
    ...
    ensureUpdatePolling(payload, remember || rememberedUpdateModal());
  }
  ...
  async function pollUpdateProgress() {
    if (state.updateOperation?.polling) return; state.updateOperation.polling = true;
    try {
      while (state.updateOperation.active) {
        let payload;
        try {
          ...
          state.updateOperation.observedStart = state.updateOperation.observedStart || (operationID && operationID !== state.updateOperation.initialOperationID) || updateActive(phase);
          if (state.updateOperation.observedStart) { if (state.updateOperation.modalIntent) rememberUpdateModal(operationID); setText('#update-progress-phase', updatePhaseLabels[phase] || phase || 'Updating…'); renderUpdateProgress(payload); renderUpdateActivity(payload); }
        }
    ...
    } finally { state.updateOperation.polling = false; }
  }
  async function startUpdateInstall(payload) {
    ...
    state.updateOperation = { active: true, polling: false, observedStart: false, modalIntent: true, initialOperationID: String(payload?.state?.operation_id || '') }; rememberUpdateModal(); const modal = $('#update-progress-modal'); if (modal) { modal.hidden = false; document.body.classList.add('modal-open'); }
    ...
    catch (_) { setText('#update-progress-phase', 'Install request interrupted; waiting for the updater to reconnect…'); }
  }
  ...
  function renderRequestTable() {
    ...
    filteredRequests().forEach((request) => {
      const row = document.createElement('tr'); row.className = 'data-row'; row.dataset.requestId = request.id; row.tabIndex = 0; row.setAttribute('role', 'button'); row.setAttribute('aria-controls', 'request-detail'); row.setAttribute('aria-expanded', 'false');
  ...
  function showRequestDetail(id) {
    ...
    if (request.error_message || request.error_code) { const error = document.createElement('p'); error.className = 'modal-note'; error.textContent = `Request failed: ${request.error_message || request.error_code}`; drawer.append(error); } drawer.append(grid);
    ...
    (request.attempt_details || []).forEach((attempt) => {
      ...
      const statusCode = Number(attempt.http_status); const httpStatus = Number.isInteger(statusCode) && statusCode >= 100 && statusCode <= 599 ? `HTTP ${statusCode}` : '';
      const detail = document.createElement('small'); detail.textContent = attempt.error_class ? [formats, attempt.state, attemptDuration, httpStatus, `${attempt.error_class}: ${attempt.error_message || 'Provider error'}`].filter(Boolean).join(' · ') : [formats, attempt.state, attemptDuration, httpStatus, dateValue(attempt.completed_at || attempt.started_at)].filter(Boolean).join(' · ');
  ...
  async function loadModels() {
    try { const payload = await fetchJSON('/api/models'); state.models = payload.data || []; const note = $('#catalog-refresh-status'); if (note) note.textContent = payload.refresh_error ? `Catalog refresh failed: ${payload.refresh_error}` : payload.updated_at && !payload.updated_at.startsWith('0001') ? `Last scanned ${new Date(payload.updated_at).toLocaleString()}` : 'Waiting for provider discovery'; pruneModelFilters(); renderModels(); renderRouteSummary(); } catch (_) { setText('#models-table-body', 'Unable to load model catalog'); }
  }
  ...
  function filterValues(key) {
    ...
    if (key === 'tags') return uniqueSorted(state.models.flatMap((model) => routeTags(model)));
  ...
  function routeTags(route) { const tags = [...(route.tags || [])]; if (route.free && !tags.includes('free')) tags.push('free'); return uniqueSorted(tags); }
  function pruneModelFilters() { ['model', 'provider', 'tags', 'input', 'output'].forEach((key) => { const available = new Set(filterValues(key)); state.modelFilters[key] = state.modelFilters[key].filter((value) => available.has(value)); }); updateModelFilterIndicators(); }
  function routeMatchesFilters(route) {
    const filters = state.modelFilters;
    const usage = Number(route.usage_7d || 0); const tags = routeTags(route);
    return (!filters.provider.length || filters.provider.includes(route.provider)) && (!filters.tags.length || filters.tags.some((value) => tags.includes(value))) && (!filters.input.length || filters.input.some((value) => (route.input_modalities || []).includes(value))) && (!filters.output.length || filters.output.some((value) => (route.output_modalities || []).includes(value))) && (filters.usage.min == null || usage >= filters.usage.min) && (filters.usage.max == null || usage <= filters.usage.max);
  }
  function filteredCatalogRows() {
    const search = ($('#models-search')?.value || '').trim().toLowerCase();
    const items = state.models.filter((route) => {
      if (state.modelFilters.model.length && !state.modelFilters.model.includes(route.model)) return false;
      if (!routeMatchesFilters(route)) return false;
      return !search || `${route.model} ${route.name || ''} ${providerName(route.provider)} ${routeTags(route).join(' ')} ${(route.input_modalities || []).join(' ')} ${(route.output_modalities || []).join(' ')}`.toLowerCase().includes(search);
    });
    const { key, direction } = state.modelSort;
    items.sort((a, b) => {
      if (!key || !direction) return Number(Boolean(b.is_new)) - Number(Boolean(a.is_new)) || String(a.model).localeCompare(String(b.model)) || providerName(a.provider).localeCompare(providerName(b.provider));
      const value = (route) => {
        if (key === 'model') return String(route.model || '').toLowerCase();
        if (key === 'provider') return providerName(route.provider).toLowerCase();
        if (key === 'modalities') return `${(route.input_modalities || []).join(' + ')} -> ${(route.output_modalities || []).join(' + ')}`.toLowerCase();
        if (key === 'tags') return routeTags(route).join(' · ').toLowerCase();
        if (key === 'price') return route.price_available && Number.isFinite(Number(route.pricing?.input)) ? Number(route.pricing.input) : null;
        if (key === 'discount') return route.discount_percent_bps == null ? null : Number(route.discount_percent_bps);
        if (key === 'usage') return Number(route.usage_7d || 0);
        if (key === 'context') return route.context_length == null ? null : Number(route.context_length);
        return null;
      };
      const av = value(a); const bv = value(b);
      if (av == null && bv == null) return String(a.model).localeCompare(String(b.model)) || providerName(a.provider).localeCompare(providerName(b.provider));
      if (av == null) return 1;
      if (bv == null) return -1;
      const result = typeof av === 'string' ? av.localeCompare(bv) : av - bv;
      return (direction === 'desc' ? -result : result) || String(a.model).localeCompare(String(b.model)) || providerName(a.provider).localeCompare(providerName(b.provider));
    });
    return items;
  }
  ...
  function cycleModelSort(key) { if (state.modelSort.key !== key) state.modelSort = { key, direction: key === 'discount' || key === 'usage' ? 'desc' : 'asc' }; else if (state.modelSort.direction === 'asc') state.modelSort.direction = 'desc'; else state.modelSort = { key: null, direction: null }; updateModelSortIndicators(); renderModels(); }
  ...
  function openModelFilter(key, button) {
    if (key === 'usage') {
      closeModelFilter(); const popover = document.createElement('div'); popover.id = 'models-filter-popover'; popover.className = 'filter-popover'; popover.setAttribute('role', 'dialog'); popover.innerHTML = '<div class="filter-popover-heading"><strong>Filter usage · last 7 days</strong><button type="button" class="filter-close" aria-label="Close filter">×</button></div><div class="usage-filter-fields"><label>Minimum requests<input data-usage-min type="number" min="0" step="1"></label><label>Maximum requests<input data-usage-max type="number" min="0" step="1"></label></div><div class="filter-option-actions"><button type="button" data-usage-unused>Unused (0)</button><button type="button" data-usage-used>Used (1+)</button><button type="button" data-filter-clear>Clear</button></div><div class="filter-popover-actions"><button type="button" class="quiet-button" data-filter-cancel>Cancel</button><button type="button" class="primary-button" data-filter-apply>Apply</button></div>'; document.body.append(popover); state.filterPopover = popover; const min = popover.querySelector('[data-usage-min]'); const max = popover.querySelector('[data-usage-max]'); min.value = state.modelFilters.usage.min ?? ''; max.value = state.modelFilters.usage.max ?? ''; popover.querySelector('.filter-close').addEventListener('click', closeModelFilter); popover.querySelector('[data-filter-cancel]').addEventListener('click', closeModelFilter); popover.querySelector('[data-filter-clear]').addEventListener('click', () => { min.value = ''; max.value = ''; }); popover.querySelector('[data-usage-unused]').addEventListener('click', () => { min.value = '0'; max.value = '0'; }); popover.querySelector('[data-usage-used]').addEventListener('click', () => { min.value = '1'; max.value = ''; }); popover.querySelector('[data-filter-apply]').addEventListener('click', () => { const nextMin = min.value === '' ? null : Math.max(0, Number(min.value)); const nextMax = max.value === '' ? null : Math.max(0, Number(max.value)); if (nextMin != null && nextMax != null && nextMin > nextMax) return; state.modelFilters.usage = { min: nextMin, max: nextMax }; closeModelFilter(); updateModelFilterIndicators(); renderModels(); }); popover.hidden = false; const rect = button.getBoundingClientRect(); popover.style.left = `${Math.max(12, Math.min(rect.left, window.innerWidth - 330))}px`; popover.style.top = `${Math.min(rect.bottom + 8, window.innerHeight - 300)}px`; return;
    }
  ...
  function updateModelFilterIndicators() { $$('.table-filter').forEach((button) => { const key = button.dataset.filterKey; const count = key === 'modalities' ? state.modelFilters.input.length + state.modelFilters.output.length : key === 'usage' ? Number(state.modelFilters.usage.min != null) + Number(state.modelFilters.usage.max != null) : state.modelFilters[key]?.length || 0; button.classList.toggle('active', count > 0); const badge = button.querySelector('.filter-count'); if (badge) { badge.textContent = count || ''; badge.hidden = count === 0; } }); const clear = $('#models-clear-filters'); if (clear) clear.disabled = !(['model', 'provider', 'tags', 'input', 'output'].some((key) => state.modelFilters[key].length) || state.modelFilters.usage.min != null || state.modelFilters.usage.max != null); }
  function clearModelFilters() { ['model', 'provider', 'tags', 'input', 'output'].forEach((key) => { state.modelFilters[key] = []; }); state.modelFilters.usage = { min: null, max: null }; closeModelFilter(); updateModelFilterIndicators(); renderModels(); }
  function appendCompactPrice(container, route) {
    const line = document.createElement('div'); line.className = 'compact-price-line'; if (route.free) { const badge = document.createElement('span'); badge.className = 'badge free price-free'; badge.textContent = 'FREE'; badge.title = 'Free route'; line.append(badge); container.append(line); return; } const part = (key, label) => { const wrapper = document.createElement('span'); wrapper.className = 'compact-price-part'; const name = document.createElement('small'); name.textContent = label; const current = Number(route.pricing?.[key] || 0); const official = Number(route.official_pricing?.[key] || 0); if (!route.price_available) { wrapper.append(name, document.createTextNode('—')); return wrapper; } if (official > current) { const original = document.createElement('s'); original.textContent = formatUSDPerMillion(official); const arrow = document.createElement('span'); arrow.className = 'price-arrow'; arrow.textContent = '→'; const discounted = document.createElement('strong'); discounted.textContent = formatUSDPerMillion(current); wrapper.append(name, original, arrow, discounted); } else { const value = document.createElement('strong'); value.textContent = formatUSDPerMillion(current); wrapper.append(name, value); } return wrapper; }; line.append(part('input', 'in'), part('output', 'out')); container.append(line);
  }
  function renderModels() {
    ensureCatalogHeaders(); const body = $('#models-table-body'); const empty = $('#models-empty'); if (!body) return; const rows = filteredCatalogRows(); body.replaceChildren(); const uniqueModels = new Set(state.models.map((route) => route.model)); const visibleModels = new Set(rows.map((route) => route.model)); const newCount = new Set(state.models.filter((route) => route.is_new).map((route) => route.model)).size; setText('#topbar-model-count', `${formatNumber(uniqueModels.size)} Models`); const newCountBadge = $('#topbar-new-model-count'); newCountBadge.textContent = `${newCount} new`; newCountBadge.hidden = newCount === 0; setText('#catalog-count', `${formatNumber(visibleModels.size)} model${visibleModels.size === 1 ? '' : 's'}`); setText('#catalog-free-count', `${formatNumber(rows.length)} routes`); setText('#catalog-free-note', `${formatNumber(rows.filter((route) => route.free).length)} free routes`); setText('#models-saved', formatUSD(state.summary?.saved_cost_pico_usd)); setText('#models-saved-percent', state.summary?.saved_percent_bps == null ? 'No baseline' : `${discountPercent(state.summary.saved_percent_bps)} saved`);
    rows.forEach((routeModel) => { const row = document.createElement('tr'); const model = appendTextCell(row, ''); model.className = 'model-cell'; const titleLine = document.createElement('div'); titleLine.className = 'model-title-line'; const displayName = routeModel.name || routeModel.model; const strong = document.createElement('strong'); strong.textContent = displayName; const quick = document.createElement('button'); quick.type = 'button'; quick.className = 'model-quick-filter'; quick.dataset.quickModelFilter = routeModel.model; quick.setAttribute('aria-label', `Show all providers for ${displayName}`); quick.title = 'Show all providers for this model'; quick.innerHTML = '<svg viewBox="0 0 16 16" aria-hidden="true"><path d="M2.5 3.25h11L9.25 8.1v3.7l-2.5 1.2V8.1L2.5 3.25Z"></path><path d="M11.3 10.7v3M9.8 12.2h3"></path></svg>'; titleLine.append(strong, quick); if (routeModel.model && routeModel.model !== displayName) { const small = document.createElement('small'); small.className = 'model-id'; small.textContent = routeModel.model; titleLine.append(small); } model.append(titleLine); const provider = document.createElement('small'); provider.className = 'model-provider'; provider.textContent = providerName(routeModel.provider); model.append(provider); if (routeModel.is_new) { row.classList.add('new-model-row'); const added = routeModel.added_at ? new Date(routeModel.added_at) : null; const label = added && !Number.isNaN(added.getTime()) ? `Added ${String(added.getDate()).padStart(2, '0')}.${String(added.getMonth() + 1).padStart(2, '0')}` : 'Added recently'; row.title = label; row.setAttribute('aria-label', `${displayName}, ${label}`); }
      const modalities = appendTextCell(row, ''); modalities.append(modalityDisplay(routeModel.input_modalities || [], routeModel.output_modalities || [])); modalities.className = 'modality-cell'; const tags = appendTextCell(row, routeTags(routeModel).join(' · ') || '—'); tags.className = 'tag-cell'; const pricing = appendTextCell(row, ''); pricing.className = 'pricing-cell'; appendCompactPrice(pricing, routeModel); pricing.title = routeModel.free ? 'Free route' : 'Input / output per 1M tokens; crossed-out values are reference prices'; const discount = appendTextCell(row, ''); discount.className = 'discount-cell'; discount.textContent = routeModel.discount_percent_bps == null ? '—' : discountPercent(routeModel.discount_percent_bps); discount.title = routeModel.discount_percent_bps == null ? 'No reference baseline' : `Input ${discountPercent(routeModel.discount_input_percent_bps)} · output ${discountPercent(routeModel.discount_output_percent_bps)}`; const usageValue = Number(routeModel.usage_7d || 0); const usage = appendTextCell(row, formatUsage(usageValue)); usage.className = 'usage-cell'; usage.title = `${formatNumber(usageValue)} requests attributed to this provider route in the last 7 days`; const context = appendTextCell(row, routeModel.context_length ? formatCompact(routeModel.context_length) : '—'); context.title = routeModel.context_length ? `${formatNumber(routeModel.context_length)} tokens` : ''; body.append(row); }); empty.hidden = rows.length > 0; updateModelSortIndicators(); updateModelFilterIndicators();
  }
  function quickFilterModel(modelID) { const search = $('#models-search'); if (search) search.value = ''; clearModelFilters(); state.modelFilters.model = [modelID]; renderModels(); }
  document.addEventListener('click', (event) => { const quick = event.target.closest('[data-quick-model-filter]'); if (!quick) return; event.preventDefault(); event.stopPropagation(); quickFilterModel(quick.dataset.quickModelFilter); });
  function ensureCatalogHeaders() { const row = document.querySelector('[data-view-panel="models"] thead tr'); if (!row || row.dataset.catalogHeadersReady) return; const price = row.querySelector('[data-sort-key="input"]'); if (price) { price.dataset.sortKey = 'price'; price.innerHTML = 'Pricing <span class="sort-indicator" aria-hidden="true">↕</span>'; } const context = row.querySelector('[data-sort-key="context"]')?.closest('th'); if (!context) return; const make = (sortKey, label, filterKey) => { const th = document.createElement('th'); const control = document.createElement('div'); control.className = 'table-header-control'; const sort = document.createElement('button'); sort.type = 'button'; sort.className = 'table-sort'; sort.dataset.sortKey = sortKey; sort.innerHTML = `${label} <span class="sort-indicator" aria-hidden="true">↕</span>`; sort.addEventListener('click', () => cycleModelSort(sortKey)); control.append(sort); if (filterKey) { const filter = document.createElement('button'); filter.type = 'button'; filter.className = 'table-filter'; filter.dataset.filterKey = filterKey; filter.setAttribute('aria-label', `Filter ${label.toLowerCase()}`); filter.title = `Filter ${label.toLowerCase()}`; filter.innerHTML = '<span class="filter-glyph" aria-hidden="true"></span><span class="filter-count" hidden></span>'; filter.addEventListener('click', (event) => { event.stopPropagation(); openModelFilter(filterKey, filter); }); control.append(filter); } th.append(control); return th; }; row.insertBefore(make('discount', 'Discount'), context); row.insertBefore(make('usage', 'Usage · 7d', 'usage'), context); row.dataset.catalogHeadersReady = 'true'; }
  ...
  function updateProviderFormMode() { ensureSubscriptionFields(); const custom = $('#provider-type')?.value === 'custom'; const fields = $('#custom-provider-fields'); const name = $('#provider-name'); const baseURL = $('#provider-base-url'); if (fields) fields.hidden = !custom; if (name) { name.required = custom; if (!custom) name.value = ''; } if (baseURL) { baseURL.required = custom; if (!custom) baseURL.value = ''; } const access = $('#provider-access-mode'); const selected = $('#provider-type')?.selectedOptions?.[0]; if (access && selected?.dataset.defaultAccessMode) access.value = selected.dataset.defaultAccessMode; const mode = access?.value || 'api'; const subscription = $('#subscription-fields'); if (subscription) subscription.hidden = mode !== 'subscription'; const fee = $('#subscription-fee'); if (fee) fee.required = mode === 'subscription'; }
  ...
  async function loadProviders() { try { const payload = await fetchJSON('/api/providers/credentials'); state.providers = payload.data || []; const list = $('#provider-list'); list.replaceChildren(); state.providers.forEach((credential) => { const row = document.createElement('div'); row.className = 'credential-row'; const icon = document.createElement('span'); icon.className = `credential-icon ${credential.provider}`; icon.textContent = credential.provider === 'surplus' ? 'S' : credential.provider === 'openrouter' ? 'O' : 'P'; row.append(icon); const main = document.createElement('span'); main.className = 'credential-main'; const strong = document.createElement('strong'); strong.textContent = credential.label || providerName(credential.provider); const small = document.createElement('small'); small.textContent = `${providerName(credential.provider)} · ${credential.enabled ? 'Enabled' : 'Disabled'}${credential.base_url ? ` · ${credential.base_url}` : ''}`; main.append(strong, small); row.append(main); const edit = document.createElement('button'); edit.type = 'button'; edit.dataset.editProvider = credential.id; edit.textContent = 'Edit'; row.append(edit); const remove = document.createElement('button'); remove.type = 'button'; remove.dataset.removeProvider = credential.id; remove.textContent = 'Remove'; row.append(remove); list.append(row); }); if (!state.providers.length) { const empty = document.createElement('div'); empty.className = 'empty-state'; empty.textContent = 'No providers connected yet.'; list.append(empty); } } catch (_) { setText('#provider-list', 'Unable to load provider credentials'); } }
  ...
  $('#key-form').addEventListener('submit', async (event) => { event.preventDefault(); const feedback = $('#key-feedback'); if (feedback) { feedback.hidden = true; feedback.textContent = ''; } const harness = $('#key-harness').value === 'Other' ? $('#key-harness-other').value.trim() : $('#key-harness').value; if (!harness) { $('#key-harness-other').focus(); return; } try { const payload = await fetchJSON('/api/client-keys', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ label: $('#key-label').value, harness }) }); $('#key-form').reset(); updateHarnessFormMode(); showNewKey(payload.secret); await loadKeys(); } catch (error) { if (feedback) { feedback.hidden = false; feedback.textContent = error.message; } } });
  ...
  function updateHarnessFormMode() { const other = $('#key-harness')?.value === 'Other'; const fields = $('#other-harness-fields'); const input = $('#key-harness-other'); if (fields) fields.hidden = !other; if (input) input.required = other; }
  ...
  $('#provider-edit-form').addEventListener('submit', async (event) => { event.preventDefault(); const id = $('#provider-edit-id').value; const label = $('#provider-edit-name').value.trim(); const feedback = $('#provider-edit-feedback'); feedback.hidden = true; try { await fetchJSON(`/api/providers/credentials/${encodeURIComponent(id)}`, { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ label }) }); closeModal('provider-edit-modal'); await loadProviders(); } catch (error) { feedback.hidden = false; feedback.className = 'provider-feedback error'; feedback.textContent = error.message; } });
