(() => {
  ...
  function showRequestDetail(id) {
    ...
    (request.attempt_details || []).forEach((attempt) => {
      ...
      const detail = document.createElement('small'); detail.textContent = attempt.error_class ? [attempt.state, attemptDuration, httpStatus, `${attempt.error_class}: ${attempt.error_message || 'Provider error'}`].filter(Boolean).join(' · ') : [attempt.state, attemptDuration, httpStatus, dateValue(attempt.completed_at || attempt.started_at)].filter(Boolean).join(' · ');
