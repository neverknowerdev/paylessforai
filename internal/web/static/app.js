(() => {
  const state = { requests: [], models: [], modelStats: [], providerStats: [], groups: [], summary: {}, view: 'overview', detailDrawer: null, modelSort: { key: null, direction: null }, modelFilters: { model: [], provider: [], tags: [], input: [], output: [] }, filterPopover: null };
  ...
  function stageSourceOptions(stage, index) {
    const source = stage.sources?.[0] || { kind: 'model', model_id: '' };
    const modelOptions = state.models.map((model) => `<option value="${String(model.model || '').replaceAll('"', '&quot;')}">${String(model.model || '').replaceAll('<', '&lt;')}</option>`).join('');
    const groupOptions = state.groups.filter((group) => group.id !== $('#group-id')?.value).map((group) => `<option value="${String(group.id).replaceAll('"', '&quot;')}">${String(group.name).replaceAll('<', '&lt;')} (${String(group.slug).replaceAll('<', '&lt;')})</option>`).join('');
    const card = document.createElement('article');
    card.className = 'group-stage-card';
    card.dataset.stageIndex = index;
    card.innerHTML = `<div class="stage-card-heading"><strong>TRY ${index + 1}</strong><button type="button" class="quiet-button remove-stage">Remove</button></div><label>Stage name<input class="stage-name" maxlength="120"></label><label>Source type<select class="source-kind"><option value="model">Models</option><option value="group">Another group</option></select></label><label class="source-value-label">Models<select class="source-value" multiple>${modelOptions}</select></label><label class="stage-billing">Access <span><label><input type="checkbox" value="free"> Free</label><label><input type="checkbox" value="subscription"> Subscription</label><label><input type="checkbox" value="metered"> Metered API</label></span></label><div class="stage-limit-grid"><label>Max input $ / 1M<input class="stage-input-limit" type="number" min="0" step="0.000001" placeholder="No limit"></label><label>Max output $ / 1M<input class="stage-output-limit" type="number" min="0" step="0.000001" placeholder="No limit"></label><label>Max expected $ / request<input class="stage-total-limit" type="number" min="0" step="0.000001" placeholder="No limit"></label></div><label>Retries after first attempt<input class="stage-retries" type="number" min="0" max="5" step="1"></label><small class="stage-explanation">Choose the lowest expected-cost route that satisfies these limits.</small>`;
    card.querySelector('.stage-name').value = stage.name || '';
    card.querySelector('.source-kind').value = source.kind || 'model';
    const sourceValue = card.querySelector('.source-value');
    if (source.kind === 'group') {
      card.querySelector('.source-value-label').childNodes[0].textContent = 'Group';
      sourceValue.multiple = false;
      sourceValue.innerHTML = groupOptions;
      sourceValue.value = source.group_id || '';
    } else {
      sourceValue.value = (stage.sources || []).map((item) => item.model_id);
    }
    card.querySelectorAll('.stage-billing input').forEach((input) => { input.checked = (stage.billing_classes || ['free', 'subscription', 'metered']).includes(input.value); });
    card.querySelector('.stage-input-limit').value = picoPerTokenToUSDPerMillion(stage.maximum_input_pico_usd_per_token);
    card.querySelector('.stage-output-limit').value = picoPerTokenToUSDPerMillion(stage.maximum_output_pico_usd_per_token);
    card.querySelector('.stage-total-limit').value = picoToUSD(stage.maximum_expected_cost_pico_usd);
    card.querySelector('.stage-retries').value = stage.same_route_retries ?? 0;
    return card;
  }
  function renderGroupStages(definition) { const container = $('#group-stage-list'); if (!container) return; container.replaceChildren(); (definition.stages || []).forEach((stage, index) => container.append(stageSourceOptions(stage, index))); if (!definition.stages?.length) container.append(stageSourceOptions(blankGroup().stages[0], 0)); }
  function collectGroupDefinition() { const stages = $$('#group-stage-list .group-stage-card').map((card, index) => { const kind = card.querySelector('.source-kind').value; const value = [...card.querySelector('.source-value').selectedOptions].map((option) => option.value).filter(Boolean); const sources = kind === 'group' ? [{ kind: 'group', group_id: value[0] || '' }] : value.map((model) => ({ kind: 'model', model_id: model })); const stage = { position: index, name: card.querySelector('.stage-name').value, sources, billing_classes: [...card.querySelectorAll('.stage-billing input:checked')].map((input) => input.value), selection: 'lowest_expected_cost', same_route_retries: Number(card.querySelector('.stage-retries').value || 0) }; const inputLimit = usdPerMillionToPico(card.querySelector('.stage-input-limit').value); const outputLimit = usdPerMillionToPico(card.querySelector('.stage-output-limit').value); const totalLimit = usdToPico(card.querySelector('.stage-total-limit').value); if (inputLimit !== undefined) stage.maximum_input_pico_usd_per_token = inputLimit; if (outputLimit !== undefined) stage.maximum_output_pico_usd_per_token = outputLimit; if (totalLimit !== undefined) stage.maximum_expected_cost_pico_usd = totalLimit; return stage; }); return { id: $('#group-id').value || undefined, revision: Number($('#group-revision').value || 0), name: $('#group-name').value, slug: $('#group-slug').value, description: $('#group-description').value, enabled: $('#group-enabled').checked, stages }; }
  ...
  function navigate(view) { const valid = ['overview', 'models', 'groups', 'stats', 'requests', 'access']; state.view = valid.includes(view) ? view : 'overview'; $$('.view-panel').forEach((panel) => panel.classList.toggle('active', panel.dataset.viewPanel === state.view)); $$('.nav-item').forEach((item) => item.classList.toggle('active', item.dataset.view === state.view)); const meta = { overview: ['Overview', 'Your local routing desk at a glance.'], models: ['Models', 'Search provider routes and compare live pricing.'], groups: ['Groups', 'Callable aliases with ordered routing rules and fallbacks.'], stats: ['Statistics', 'Reliability, retries, response time, and cost by model.'], requests: ['Requests', 'Detailed usage, cost, and request outcomes.'], access: ['Access & keys', 'Manage the keys and providers behind your proxy.'] }[state.view]; setText('#page-title', meta[0]); setText('#page-kicker', meta[0].toUpperCase()); setText('#page-description', meta[1]); $('#sidebar').classList.remove('open'); if (window.location.hash !== `#${state.view}`) history.replaceState(null, '', `#${state.view}`); }
