(() => {
  ...
  function closeRemoteAuthModal() { const modal = $('#remote-auth-modal'); if (modal) closeModal(modal); }
  ...
  function syncRemoteAccessUX(status) { const mode = status.desired_mode || 'disabled'; if (mode === 'disabled' || status.phase === 'online') closeRemoteAuthModal(); }
  ...
  function setupRemoteAccessUX() {
    ...
    if (!$('#access-mode-card')) {
      ...
      if (remoteAccessCard) {
        ...
        const authLink = $('#remote-access-action-link');
        authLink?.addEventListener('click', (event) => { event.preventDefault(); if (remoteAccessStatus) openRemoteAuthModal(remoteAccessStatus); });
      }
    }
    if (listener) {
