(() => {
  ...
  function renderStatsOverview() {
    ...
    const items = [['REQUESTS', formatNumber(s.total_requests), `${formatNumber(s.succeeded_requests || 0)} succeeded · ${formatNumber(s.failed_requests || 0)} failed`, ''], ['ATTEMPTS', formatNumber(s.total_attempts || 0), `${formatNumber(retried)} request${retried === 1 ? '' : 's'} retried`, 'accent-blue'], ['SUCCESS RATE', s.total_requests ? `${success}%` : '—', 'Terminal request outcomes', 'accent-green'], ['FASTEST', formatDuration(s.fastest_response_ms), 'Completed response', 'accent-lime'], ['AVERAGE', formatDuration(s.average_response_ms), `${formatNumber(s.requests_with_response_time || 0)} timed requests`, 'accent-blue'], ['SLOWEST', formatDuration(s.slowest_response_ms), 'Completed response', 'accent-peach'], ['TOKENS', formatNumber(s.total_tokens), 'Input + output', ''], ['SAVED', formatUSD(s.saved_cost_pico_usd), 'vs. official catalog price', 'accent-lime']];
