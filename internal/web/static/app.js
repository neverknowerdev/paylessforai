(() => {
  ...
  function renderModels() {
    ...
    rows.forEach((routeModel) => { const row = document.createElement('tr'); const model = appendTextCell(row, ''); model.className = 'model-cell'; const titleLine = document.createElement('div'); titleLine.className = 'model-title-line'; const strong = document.createElement('strong'); strong.textContent = routeModel.name || routeModel.model; const quick = document.createElement('button'); quick.type = 'button'; quick.className = 'model-quick-filter'; quick.dataset.quickModelFilter = routeModel.model; quick.setAttribute('aria-label', `Show all providers for ${routeModel.name || routeModel.model}`); quick.title = 'Show all providers for this model'; quick.innerHTML = '<svg viewBox="0 0 16 16" aria-hidden="true"><path d="M2.5 3.25h11L9.25 8.1v3.7l-2.5 1.2V8.1L2.5 3.25Z"></path><path d="M11.3 10.7v3M9.8 12.2h3"></path></svg>'; titleLine.append(strong, quick); const small = document.createElement('small'); small.className = 'model-id'; small.textContent = routeModel.model; titleLine.append(small); model.append(titleLine); const provider = document.createElement('small'); provider.className = 'model-provider'; provider.textContent = providerName(routeModel.provider); model.append(provider); if (routeModel.is_new) { row.classList.add('new-model-row'); const added = routeModel.added_at ? new Date(routeModel.added_at) : null; const label = added && !Number.isNaN(added.getTime()) ? `Added ${String(added.getDate()).padStart(2, '0')}.${String(added.getMonth() + 1).padStart(2, '0')}` : 'Added recently'; row.title = label; row.setAttribute('aria-label', `${routeModel.name || routeModel.model}, ${label}`); }
