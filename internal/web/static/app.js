(() => {
  ...
  let remotePollTimer;
  let remoteAuthRequested = false;
  let remoteAuthInitialized = false;
  ...
  function renderRemoteAccess(status) { const phase = status.phase || 'disabled'; const mode = status.desired_mode || 'disabled'; const transitional = ['starting', 'connecting', 'auth_required', 'stopping'].includes(phase); const phaseLabel = { disabled: 'Off', starting: 'Starting', connecting: 'Connecting', auth_required: 'Action required', online: 'Online', stopping: 'Stopping', error: 'Error' }[phase] || phase; setText('#remote-access-phase', phaseLabel); const modeControl = $('#remote-access-mode'); const host = $('#remote-access-hostname'); if (modeControl) modeControl.value = mode; if (host) { host.value = status.hostname || 'paylessforai'; host.disabled = mode !== 'disabled' || transitional || phase === 'online'; } setText('#remote-access-description', mode === 'funnel' ? 'The owner-only dashboard stays private to the authorizing Tailscale user. Only /v1 inference routes are public, and each request requires a PayLessForAI client key.' : mode === 'private' ? 'The dashboard and inference API are available to the authorizing Tailscale user on the tailnet. Inference still requires a PayLessForAI client key.' : 'The local loopback server remains available on this computer.'); const action = $('#remote-access-action'); const actionURL = remoteActionURL(status.action?.url); if (action) action.hidden = true; const link = $('#remote-access-action-link'); if (link) { link.href = actionURL || '#'; link.hidden = true; } const links = $('#remote-access-links'); if (links) links.hidden = phase !== 'online'; setText('#remote-access-dashboard', status.dashboard_url || ''); setText('#remote-access-base', status.base_url || ''); const error = $('#remote-access-error'); if (error) { error.hidden = !status.last_error; error.textContent = status.last_error?.message || ''; } const retry = $('#remote-access-retry'); if (retry) retry.hidden = phase !== 'error'; const stop = $('#remote-access-stop'); if (stop) stop.hidden = mode === 'disabled' || phase === 'disabled'; const forget = $('#remote-access-forget'); if (forget) forget.hidden = true; const save = $('#remote-access-save'); if (save) { save.hidden = true; save.disabled = transitional; } syncRemoteAccessPanels(mode); if (transitional) { clearTimeout(remotePollTimer); remotePollTimer = setTimeout(loadRemoteAccess, 2000); } }
  function ensureRemoteAuthModal() { let modal = $('#remote-auth-modal'); if (modal) return modal; modal = document.createElement('div'); modal.className = 'modal-backdrop remote-auth-modal'; modal.id = 'remote-auth-modal'; modal.hidden = true; modal.innerHTML = '<div class="modal" role="dialog" aria-modal="true" aria-labelledby="remote-auth-modal-title"><div class="modal-heading"><div><span class="section-label">TAILSCALE SETUP</span><h3 id="remote-auth-modal-title">Authorize this device</h3></div><button class="icon-button" type="button" data-remote-auth-close aria-label="Close authorization dialog">×</button></div><p id="remote-auth-modal-note" class="modal-note"></p><div class="auth-flow"><div class="auth-flow-note">Tailscale opens a secure sign-in page. Finish authorization there, then return here and check the connection.</div><a id="remote-auth-modal-link" class="primary-button" target="_blank" rel="noopener noreferrer">Open Tailscale authorization</a></div><div id="remote-auth-modal-error" class="provider-feedback" role="alert" hidden></div><div class="modal-actions"><button class="quiet-button" type="button" data-remote-auth-close>Close</button><button class="primary-button" type="button" data-remote-auth-check>I’ve authorized, check again</button></div></div>'; document.body.append(modal); modal.querySelectorAll('[data-remote-auth-close]').forEach((button) => button.addEventListener('click', () => closeRemoteAuthModal())); modal.querySelector('[data-remote-auth-check]').addEventListener('click', async () => { const check = modal.querySelector('[data-remote-auth-check]'); const error = $('#remote-auth-modal-error'); check.disabled = true; if (error) error.hidden = true; try { await fetchJSON('/api/remote-access/retry', { method: 'POST' }); await loadRemoteAccess(); } catch (requestError) { if (error) { error.textContent = requestError.message; error.hidden = false; } } finally { check.disabled = false; } }); return modal; }
  function openRemoteAuthModal(status) { const modal = ensureRemoteAuthModal(); const mode = status.desired_mode === 'funnel' ? 'Public API link' : 'Private devices'; setText('#remote-auth-modal-title', `Connect ${mode}`); setText('#remote-auth-modal-note', status.desired_mode === 'funnel' ? 'Authorize Tailscale to publish the key-protected inference API. Your dashboard remains private to your Tailscale identity.' : 'Authorize Tailscale to make the dashboard and inference API available to your private Tailscale network.'); const link = $('#remote-auth-modal-link'); const actionURL = remoteActionURL(status.action?.url); if (link) { link.href = actionURL || '#'; link.hidden = !actionURL; } const error = $('#remote-auth-modal-error'); if (error) error.hidden = true; modal.hidden = false; document.body.classList.add('modal-open'); }
  function closeRemoteAuthModal() { const modal = $('#remote-auth-modal'); if (modal) closeModal(modal); remoteAuthRequested = false; }
  function syncRemoteAccessPanels(mode) { const remoteCard = $('.remote-access-card'); const listener = $('#network-settings-form')?.closest('.settings-card'); if (remoteCard) remoteCard.hidden = mode === 'disabled'; if (listener) listener.hidden = mode !== 'disabled'; $$('.access-mode-option').forEach((option) => { option.setAttribute('aria-pressed', option.dataset.accessMode === mode ? 'true' : 'false'); }); }
  function syncRemoteAccessUX(status) { const mode = status.desired_mode || 'disabled'; if (!remoteAuthInitialized) { remoteAuthRequested = mode !== 'disabled'; remoteAuthInitialized = true; } if (mode === 'disabled' || status.phase === 'online') { if (status.phase === 'online') remoteAuthRequested = false; closeRemoteAuthModal(); return; } if (status.phase === 'auth_required' && remoteAuthRequested) openRemoteAuthModal(status); }
  async function loadRemoteAccess() { try { const status = await fetchJSON('/api/remote-access'); renderRemoteAccess(status); syncRemoteAccessUX(status); } catch (_) { setText('#remote-access-error', 'Remote-access status is unavailable'); $('#remote-access-error').hidden = false; } }
  ...
  function setupRemoteAccessUX() {
    ...
    if (remoteAccessCard) { settingsGrid.prepend(remoteAccessCard); remoteAccessCard.hidden = true; }
    if (listener) {
      settingsGrid.append(listener);
      if (listenerPanel && listenerPanel !== settingsPanel) listenerPanel.remove();
    }
    const legacyMode = $('#remote-access-mode');
    if (legacyMode) { legacyMode.hidden = true; legacyMode.closest('label')?.setAttribute('hidden', ''); }
    if (!$('#access-mode-card')) {
    ...
    if (listener) {
      ...
      const anchor = remoteAccessCard || modeCard?.nextElementSibling;
